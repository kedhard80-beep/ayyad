...
const CASES = [
  ...
  { id:5, title:{fr:"Traitement neurologique pour Mariam",en:"Neurological treatment for Mariam"}, beneficiary:"Mariam Ouédraogo", age:41, city:"Abidjan", hospital:"CHU de Yopougon", category:{fr:"Cardiologie",en:"Cardiology"}, required:1100000, collected:330000, donors:22, daysLeft:33, image:"🧠", desc:{fr:"Mariam souffre d'une sclérose en plaques progressivement invalidante. Un traitement de fond biologique est disponible mais inaccessible sans aide.",en:"Mariam suffers from progressively disabling multiple sclerosis. A biological treatment is available but inaccessible without help."}, status:"COLLECTING" },
...
const HomePage = ({ setPage, setSelectedCase, lang }) => {
  ...
  const filtered = filter==="all"||filter===catMap[0] ? CASES : CASES.filter(c => c.category[lang].toLowerCase()===filter.toLowerCase());
  return (
    <div>
      ...
      <div id="collectes" className="max-w-6xl mx-auto px-4 py-12">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div><h2 className="text-2xl font-black text-gray-900">{t.collections.title}</h2><p className="text-gray-500 text-sm mt-1">{CASES.filter(c=>c.status==="COLLECTING").length} {t.collections.sub}</p></div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 flex items-center justify-center px-4 py-16">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md p-8">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {t.roles.map(r => <button key={r.id} onClick={() => setRole(r.id)} className={`py-3 rounded-xl border text-xs font-bold transition-all flex flex-col items-center gap-1 ${role===r.id?"bg-emerald-600 text-white border-emerald-600 shadow-md":"bg-gray-50 text-gray-600 border-gray-200 hover:border-emerald-300"}`}><span className="text-xl">{r.icon}</span>{r.label}</button>)}
          </div>
        </div>
        <div className="space-y-4 mb-6">
          ...
          <div><label className="text-xs font-semibold text-gray-600 mb-1.5 block">{t.password}</label><input value={pwd} onChange={e=>setPwd(e.target.value)} type="password" placeholder="••••••••" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" /></div>
        </div>
        <button onClick={handleLogin} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl shadow-md text-sm">{t.btn}</button>
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 flex items-center justify-center px-4 py-16">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md p-8">
        ...
        {step===2&&<div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2"><button onClick={()=>setStep(1)} className="border border-gray-200 text-gray-600 font-semibold py-3 rounded-xl text-sm">{t.back}</button><button onClick={handleSubmit} className="bg-emerald-600 text-white font-bold py-3 rounded-xl text-sm shadow-md">{t.btn}</button></div>
        </div>}
      </div>
    </div>
  );
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        {step===2&&<div className="space-y-4">
          ...
          {t.docs.map(doc=><div key={doc.key} className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all ${uploaded[doc.key]?"border-emerald-300 bg-emerald-50":"border-gray-200"}`}><div className={`w-11 h-11 rounded-xl flex items-center justify-center text-2xl flex-shrink-0 ${uploaded[doc.key]?"bg-emerald-100":"bg-gray-100"}`}>{doc.icon}</div><div className="flex-1 min-w-0"><div className="font-semibold text-sm text-gray-900">{doc.title} <span className="text-red-400">*</span></div><div className="text-xs text-gray-500">{doc.desc}</div></div><button onClick={()=>setUploaded(u=>({...u,[doc.key]:true}))} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex-shrink-0 ${uploaded[doc.key]?"bg-emerald-600 text-white":"bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>{uploaded[doc.key]?t.uploaded:t.upload}</button></div>)}
          ...
          <div className="grid grid-cols-2 gap-2"><button onClick={()=>setStep(1)} className="border border-gray-200 text-gray-600 font-semibold py-3 rounded-xl text-sm">{t.back}</button><button onClick={()=>setStep(3)} disabled={!allUploaded} className="bg-emerald-600 disabled:bg-gray-200 disabled:text-gray-400 text-white font-bold py-3 rounded-xl text-sm shadow-md">{t.submit}</button></div>
        </div>}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {tab==="overview"&&<div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6"><h3 className="font-bold text-gray-900 mb-4">{t.recentTitle}</h3><div className="space-y-3">{CASES.slice(0,4).map(c=><div key={c.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><span className="text-2xl">{c.image}</span><div className="flex-1 min-w-0"><div className="text-sm font-semibold text-gray-900 truncate">{c.title[lang]}</div><div className="flex items-center gap-2 mt-0.5"><div className="text-xs text-gray-400">{fmt(c.required)}</div><div className="flex-1 max-w-[80px]"><ProgressBar percent={pct(c.collected,c.required)}/></div><div className="text-xs text-emerald-600 font-bold">{pct(c.collected,c.required)}%</div></div></div><Badge color={c.status==="COLLECTING"?"blue":"green"}>{c.status==="COLLECTING"?t.active2:t.funded}</Badge></div>)}</div></div>
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6"><h3 className="font-bold text-gray-900 mb-4">{t.revenueTitle}</h3><div className="space-y-3">{t.months.map(r=><div key={r.month} className="p-4 bg-gray-50 rounded-xl"><div className="flex justify-between items-center mb-2"><span className="font-semibold text-sm text-gray-700">{r.month}</span><Badge color="green">{r.fees}</Badge></div><div className="text-xs text-gray-400">{lang==="fr"?"Dons : ":"Donations: "}{r.dons} FCFA</div></div>)}</div></div>
          </div>
        </div>}
        {tab==="cases"&&<div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
