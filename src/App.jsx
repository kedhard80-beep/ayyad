...
const T = {
  fr: {
    ...
    submit: {
      title: "Soumettre un dossier", steps: ["Informations","Documents","Confirmation"],
      infoTitle: "Décrivez votre situation médicale",
      titleField: "Titre de la collecte *", descField: "Description *", hospitalField: "Hôpital *",
      cityField: "Ville *", amountField: "Montant du devis (FCFA) *", categoryField: "Spécialité",
      cats: ["Cardiologie","Oncologie","Neurologie","Orthopédie","Pédiatrie","Gynécologie","Autre"],
      next: "Continuer →",
      docsTitle: "Documents requis", docsSub: "Tous les documents sont chiffrés (AES-256).",
      docs: [
        { key:"medical", icon:"📄", title:"Rapport médical", desc:"Compte-rendu ou ordonnance du médecin" },
        { key:"quote", icon:"🏥", title:"Devis hospitalier", desc:"Devis officiel signé par l'établissement" },
        { key:"id", icon:"🪪", title:"Pièce d'identité", desc:"CNI, passeport ou titre de séjour valide" },
        { key:"consent", icon:"✍️", title:"Consentement données", desc:"Formulaire Ayyad de consentement" }
      ],
      upload: "Choisir fichier", uploaded: "✓ Envoyé", uploading: "Envoi...", uploadError: "Erreur, réessayez",
      warning: "Tous les documents sont obligatoires pour la vérification.",
      back: "← Retour", submit: "Soumettre →",
      successTitle: "Dossier soumis !", successSub: "Votre dossier est en cours d'examen.",
      processSteps: ["Dossier reçu et numéroté","Vérification équipe Ayyad (< 48h)","Contact hôpital pour validation devis","Mise en ligne de la collecte"],
      backHome: "Retour à l'accueil",
      submitting: "Envoi en cours...", submitError: "Erreur lors de la soumission. Réessayez."
    },
    login: { title: "Connexion à Ayyad", sub: "Bienvenue ! Connectez-vous à votre espace.", emailLabel: "Email", passwordLabel: "Mot de passe", btn: "Se connecter →", noAccount: "Pas encore de compte ?", register: "S'inscrire", error: "Email ou mot de passe incorrect.", loading: "Connexion..." },
    register: {
      title: "Créer un compte", roleQ: "Je souhaite...",
      roles: [{ id:"donor",icon:"💚",title:"Faire des dons",desc:"Aider des patients dans le besoin" },{ id:"beneficiary",icon:"🏥",title:"Recevoir des soins",desc:"Financer une intervention médicale" }],
      fields: [{ key:"name",label:"Nom complet",p:"Aminata Koné",type:"text" },{ key:"email",label:"Email",p:"vous@exemple.ci",type:"email" },{ key:"phone",label:"Numéro Wave / Orange Money",p:"+225 07 XX XX XX XX",type:"tel" },{ key:"password",label:"Mot de passe",p:"••••••••",type:"password" }],
      ...
      error: "Erreur lors de la création du compte.", loading: "Création..."
    },
    admin: {
      ...
      stats: [{ label:"Dossiers actifs",v:"23",icon:"📋" },{ label:"Dons ce mois",v:"12.4M FCFA",icon:"💚" },{ label:"Bénéficiaires aidés",v:"142",icon:"🏥" }],
      recentTitle: "Collectes récentes", revenueTitle: "Revenus opérationnels (5%)",
      ...
      pendingTitle: "Dossiers en attente de validation", pending: "en attente", empty: "Tous les dossiers ont été traités",
      risk: "Risque", reject: "Rejeter", approve: "Approuver ✓",
      ...
      resolve: "Résoudre", resolved: "Résolu", payoutsTitle: "Virements hospitaliers", payoutsPending: "en attente",
      validate: "Valider →", active2: "Actif", funded: "Financé",
      submittedCases: "Dossiers soumis", noSubmissions: "Aucun dossier soumis pour le moment.", loadingCases: "Chargement..."
    },
    howPage: {
      title: "Comment fonctionne Ayyad ?", sub: "Transparent, sécurisé, conçu pour l'Afrique",
      forDonors: { icon:"💚",title:"Pour les donateurs",steps:["Parcourez les collectes vérifiées actives","Choisissez librement votre montant","Payez via Wave, Orange Money ou carte","Vous êtes débité exactement du montant choisi","L'argent arrive directement à l'hôpital"] },
      forBenef: { icon:"🏥",title:"Pour les bénéficiaires",steps:["Créez un compte et soumettez votre dossier médical","Téléchargez rapport médical, devis, pièce d'identité","Notre équipe vérifie avec l'hôpital partenaire","Votre collecte est mise en ligne sous 48h","Les fonds sont versés directement à l'hôpital"] },
      feeTitle: "La règle des 5% — Toujours silencieuse", feeSub: "Ayyad prélève une commission opérationnelle de 5% sur chaque don. Cette commission est totalement invisible pour vous.",
  ...
  en: {
    ...
    submit: {
      title: "Submit a medical case", steps: ["Information","Documents","Confirmation"],
      infoTitle: "Describe your medical situation",
      titleField: "Campaign title *", descField: "Description *", hospitalField: "Hospital *",
      cityField: "City *", amountField: "Quoted amount (FCFA) *", categoryField: "Specialty",
      cats: ["Cardiology","Oncology","Neurology","Orthopedics","Pediatrics","Gynecology","Other"],
      next: "Continue →",
      docsTitle: "Required documents", docsSub: "All documents are encrypted (AES-256).",
      docs: [
        { key:"medical", icon:"📄", title:"Medical report", desc:"Doctor's report or prescription" },
        { key:"quote", icon:"🏥", title:"Hospital quote", desc:"Official quote signed by the institution" },
        { key:"id", icon:"🪪", title:"Identity document", desc:"Valid national ID, passport or residence permit" },
        { key:"consent", icon:"✍️", title:"Data consent", desc:"Ayyad consent form" }
      ],
      upload: "Choose file", uploaded: "✓ Uploaded", uploading: "Uploading...", uploadError: "Error, try again",
      warning: "All documents are required for verification.",
      back: "← Back", submit: "Submit →",
      successTitle: "Case submitted!", successSub: "Your case is under review.",
      processSteps: ["Case received and numbered","Ayyad team review (< 48h)","Hospital contact for quote validation","Campaign goes live"],
      backHome: "Back to home",
      submitting: "Submitting...", submitError: "Submission error. Please try again."
    },
    login: { title: "Sign in to Ayyad", sub: "Welcome! Sign in to your account.", emailLabel: "Email", passwordLabel: "Password", btn: "Sign in →", noAccount: "Don't have an account?", register: "Sign up", error: "Invalid email or password.", loading: "Signing in..." },
    register: {
      title: "Create an account", roleQ: "I want to...",
      roles: [{ id:"donor",icon:"💚",title:"Make donations",desc:"Help patients in need" },{ id:"beneficiary",icon:"🏥",title:"Receive care",desc:"Fund a medical procedure" }],
      fields: [{ key:"name",label:"Full name",p:"Aminata Koné",type:"text" },{ key:"email",label:"Email",p:"you@example.ci",type:"email" },{ key:"phone",label:"Wave / Orange Money number",p:"+225 07 XX XX XX XX",type:"tel" },{ key:"password",label:"Password",p:"••••••••",type:"password" }],
      terms: "I accept the", termsLink: "terms of service", and: "and the", privacyLink: "privacy policy",
      btn: "Create my account", continue: "Continue →", back: "← Back", hasAccount: "Already have an account?", signin: "Sign in",
      error: "Error creating account.", loading: "Creating..."
    },
    admin: {
      ...
      stats: [{ label:"Active cases",v:"23",icon:"📋" },{ label:"Donations this month",v:"12.4M FCFA",icon:"💚" },{ label:"Patients helped",v:"142",icon:"🏥" }],
      recentTitle: "Recent campaigns", revenueTitle: "Operational revenue (5%)",
      ...
      pendingTitle: "Cases pending validation", pending: "pending", empty: "All cases have been processed",
      risk: "Risk", reject: "Reject", approve: "Approve ✓",
      ...
      resolve: "Resolve", resolved: "Resolved", payoutsTitle: "Hospital payouts", payoutsPending: "pending",
      validate: "Validate →", active2: "Active", funded: "Funded",
      submittedCases: "Submitted cases", noSubmissions: "No cases submitted yet.", loadingCases: "Loading..."
    },
    howPage: {
      title: "How does Ayyad work?", sub: "Transparent, secure, built for Africa",
      forDonors: { icon:"💚",title:"For donors",steps:["Browse verified active campaigns","Freely choose your amount","Pay via Wave, Orange Money or card","You are charged exactly the amount you chose","The money goes directly to the hospital"] },
      forBenef: { icon:"🏥",title:"For beneficiaries",steps:["Create an account and submit your medical case","Upload medical report, quote, identity document","Our team verifies with the partner hospital","Your campaign goes live within 48h","Funds are transferred directly to the hospital"] },
      feeTitle: "The 5% rule — Always silent", feeSub: "Ayyad charges a 5% operational fee on each donation. This fee is completely invisible to you.",
      youGive: "You give", collectReceives: "Campaign receives", ayyadFee: "Ayyad fee (5%)"
    },
...
// ============================================================
// DEMO CASES (fallback si Supabase vide)
// ============================================================
const DEMO_CASES = [
  ...
  { id:6, title:{fr:"Opération de la vue pour Kouassi",en:"Eye surgery for Kouassi"}, beneficiary:"Kouassi Yao", age:67, city:"San-Pédro", hospital:"Clinique Vision CI", category:{fr:"Oncologie",en:"Oncology"}, required:380000, collected:285000, donors:41, daysLeft:8, image:"👁️", desc:{fr:"Kouassi souffre de glaucome bilatéral avancé.",en:"Kouassi suffers from advanced bilateral glaucoma."}, status:"COLLECTING" },
];
...
const Badge = ({ children, color="green" }) => {
  const map = { green:"bg-emerald-100 text-emerald-700", yellow:"bg-amber-100 text-amber-700", red:"bg-red-100 text-red-700", blue:"bg-blue-100 text-blue-700", gray:"bg-gray-100 text-gray-600" };
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:block text-sm font-medium text-gray-700 max-w-[100px] truncate">{user.name||user.email}</span>
    ...
    <div onClick={onClick} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-200 cursor-pointer overflow-hidden group">
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-2 mb-2">
          <h3 className="font-bold text-gray-900 text-sm leading-snug group-hover:text-emerald-700 transition-colors">{typeof c.title==="object"?c.title[lang]:c.title}</h3>
          <Badge color={funded?"green":"blue"}>{funded?t.badges.funded:(typeof c.category==="object"?c.category[lang]:c.category)||"—"}</Badge>
        </div>
        ...
        <div className="mb-3">
          <div className="flex justify-between text-xs mb-1.5">
            <span className="font-bold text-emerald-700">{fmt(c.collected||0)}</span>
        ...
        <div className="flex items-center justify-between text-xs text-gray-500">
          <span>👥 {c.donors||0} {tc.donors}</span>
          {funded?<span className="text-emerald-600 font-bold">{tc.funded}</span>:<span className="text-amber-600 font-medium">⏳ {c.daysLeft||0} {tc.daysLeft}</span>}
        </div>
      </div>
    </div>
  );
...
const HomePage = ({ setPage, setSelectedCase, lang }) => {
  ...
  const filtered = filter==="all"||filter===catMap[0] ? DEMO_CASES : DEMO_CASES.filter(c => (typeof c.category==="object"?c.category[lang]:c.category).toLowerCase()===filter.toLowerCase());
  return (
    <div>
      ...
      <div id="collectes" className="max-w-6xl mx-auto px-4 py-12">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div><h2 className="text-2xl font-black text-gray-900">{t.collections.title}</h2><p className="text-gray-500 text-sm mt-1">{DEMO_CASES.filter(c=>c.status==="COLLECTING").length} {t.collections.sub}</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              <div className="flex flex-wrap gap-2 mb-3"><Badge color="blue">{typeof c.category==="object"?c.category[lang]:c.category}</Badge><Badge color="green">{t.badges.verified}</Badge>{funded&&<Badge color="green">{t.badges.funded}</Badge>}</div>
              <h1 className="text-2xl font-black text-gray-900 mb-3">{title}</h1>
              <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-4"><span>🏥 {c.hospital}</span><span>📍 {c.city}</span>{c.age&&<span>👤 {c.age} {lang==="fr"?"ans":"years"}</span>}</div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <div className="flex justify-between items-end mb-3">
              <div><div className="text-3xl font-black text-emerald-700">{fmt(c.collected||0)}</div><div className="text-sm text-gray-500">{t.progress.collected} {fmt(c.required)}</div></div>
            ...
            <div className="flex justify-between mt-3 text-sm text-gray-500">
              <span>👥 {c.donors||0} {t.progress.donors}</span>
              {funded?<span className="text-emerald-600 font-semibold">{t.progress.intervention}</span>:<span className="text-amber-600 font-medium">⏳ {c.daysLeft||0} {t.progress.daysLeft}</span>}
            </div>
          </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-xl p-6 sticky top-24">
            {step==="donate" && <>
              ...
              <button onClick={() => setStep("confirm")} disabled={!amount||isNaN(amount)||Number(amount)<500||funded} className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-200 disabled:text-gray-400 text-white font-bold py-3.5 rounded-xl transition-colors shadow-md text-sm">
...
const LoginPage = ({ setPage, setUser, lang }) => {
  ...
  const handleLogin = async () => {
    ...
    setUser({ name: data.user.user_metadata?.name || email, email: data.user.email, role: data.user.user_metadata?.role || "donor" });
    setPage("home");
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 flex items-center justify-center px-4 py-16">
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md p-8">
        ...
        {error && <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl p-3 text-sm mb-4 text-center">{error}</div>}
        <div className="space-y-4 mb-6">
          <div><label className="text-xs font-semibold text-gray-600 mb-1.5 block">{t.emailLabel}</label><input value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="vous@exemple.ci" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" /></div>
          <div><label className="text-xs font-semibold text-gray-600 mb-1.5 block">{t.passwordLabel}</label><input value={pwd} onChange={e=>setPwd(e.target.value)} type="password" placeholder="••••••••" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" onKeyDown={e=>e.key==="Enter"&&handleLogin()} /></div>
        </div>
        <button onClick={handleLogin} disabled={loading} className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-300 text-white font-bold py-3.5 rounded-xl shadow-md text-sm">
      ...
      <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md p-8">
        ...
        {step===2&&<div className="space-y-4">
          ...
          <div className="flex items-start gap-2 text-xs text-gray-500">
            <input type="checkbox" checked={termsAccepted} onChange={e=>setTermsAccepted(e.target.checked)} className="mt-0.5 accent-emerald-600" />
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button onClick={handleSubmit} disabled={loading||!termsAccepted||!form.email||!form.password} className="bg-emerald-600 disabled:bg-gray-300 text-white font-bold py-3 rounded-xl text-sm shadow-md">
              {loading ? t.loading : t.btn}
            </button>
          </div>
        </div>}
...
// ============================================================
// SUBMIT PAGE — VRAIE SOUMISSION SUPABASE
// ============================================================
const SubmitPage = ({ setPage, lang, user }) => {
  const [step, setStep] = useState(1);
  const [form, setForm] = useState({ title:"", description:"", hospital:"", city:"", amount:"", category:"" });
  // fileStates : { medical: null|File, quote: null|File, id: null|File, consent: null|File }
  const [files, setFiles] = useState({ medical:null, quote:null, id:null, consent:null });
  // uploadStatus : { medical: 'idle'|'uploading'|'done'|'error', ... }
  const [uploadStatus, setUploadStatus] = useState({ medical:"idle", quote:"idle", id:"idle", consent:"idle" });
  ...
  const getStatusLabel = (docKey) => {
    const s = uploadStatus[docKey];
    if (s === "done") return t.uploaded;
    if (s === "uploading") return t.uploading;
    if (s === "error") return t.uploadError;
    return t.upload;
  };
  ...
  const getStatusColor = (docKey) => {
    ...
    if (s === "uploading") return "bg-amber-100 text-amber-700";
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-1 mb-8">
        {t.steps.map((s,i)=><div key={i} className="flex items-center gap-1 flex-1 last:flex-none">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${step>i+1?"bg-emerald-500 text-white":step===i+1?"bg-emerald-600 text-white":"bg-gray-200 text-gray-500"}`}>{step>i+1?"✓":i+1}</div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        {step===1&&<div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="text-xs font-semibold text-gray-600 mb-1.5 block">{t.categoryField}</label>
              <select value={form.category} onChange={e=>setForm({...form,category:e.target.value})} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-emerald-400">
                <option value="">—</option>{t.cats.map(c=><option key={c}>{c}</option>)}
              </select>
            </div>
          </div>
          <button onClick={()=>setStep(2)} disabled={!valid1} className="w-full bg-emerald-600 disabled:bg-gray-200 disabled:text-gray-400 text-white font-bold py-3.5 rounded-xl text-sm shadow-md">{t.next}</button>
        </div>}
        ...
        {step===2&&<div className="space-y-4">
          ...
          {t.docs.map(doc=>(
            <div key={doc.key} className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all ${uploadStatus[doc.key]==="done"?"border-emerald-300 bg-emerald-50":uploadStatus[doc.key]==="error"?"border-red-200 bg-red-50":"border-gray-200"}`}>
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-2xl flex-shrink-0 ${uploadStatus[doc.key]==="done"?"bg-emerald-100":"bg-gray-100"}`}>{doc.icon}</div>
              <div className="flex-1 min-w-0">
                ...
                {files[doc.key] && uploadStatus[doc.key]==="done" && <div className="text-xs text-emerald-600 mt-0.5 truncate">📎 {files[doc.key].name}</div>}
              </div>
              <label className={`px-3 py-1.5 rounded-xl text-xs font-bold flex-shrink-0 cursor-pointer transition-all ${getStatusColor(doc.key)}`}>
                {getStatusLabel(doc.key)}
                <input
                  type="file"
                  accept=".pdf,image/*"
                  className="hidden"
                  disabled={uploadStatus[doc.key]==="uploading"||uploadStatus[doc.key]==="done"}
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={()=>setStep(1)} className="border border-gray-200 text-gray-600 font-semibold py-3 rounded-xl text-sm">{t.back}</button>
            <button onClick={handleSubmit} disabled={!allUploaded||submitting} className="bg-emerald-600 disabled:bg-gray-200 disabled:text-gray-400 text-white font-bold py-3 rounded-xl text-sm shadow-md">
        ...
        {step===3&&<div className="text-center space-y-5 py-4">
          ...
          <div className="bg-gray-50 rounded-2xl p-5 text-left space-y-3">
            {t.processSteps.map((s,i)=><div key={i} className="flex items-center gap-3 text-sm">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${i===0?"bg-emerald-500 text-white":"bg-gray-200 text-gray-500"}`}>{i===0?"✓":i+1}</div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        {[t.forDonors,t.forBenef].map((s,i)=><div key={i} className={`bg-white border-2 rounded-2xl p-6 shadow-sm ${i===0?"border-emerald-200":"border-blue-200"}`}>
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl mb-5 border ${i===0?"bg-emerald-50 border-emerald-200":"bg-blue-50 border-blue-200"}`}><span className="text-xl">{s.icon}</span><span className="font-bold text-gray-900">{s.title}</span></div>
          <ol className="space-y-3">{s.steps.map((step,j)=><li key={j} className="flex items-start gap-3 text-sm text-gray-700"><div className="w-6 h-6 bg-gray-900 text-white rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">{j+1}</div>{step}</li>)}</ol>
        </div>)}
      </div>
...
// ============================================================
// ADMIN PAGE — avec dossiers Supabase réels
// ============================================================
  ...
  useEffect(() => {
    if(tab==="cases") {
      ...
      supabase.from("cases").select("*").order("created_at", { ascending: false })
        .then(({ data }) => { setRealCases(data||[]); setLoadingCases(false); });
    }
  }, [tab]);

  const payoutLabel = (s) => lang==="fr"?(s==="COMPLETED"?"✓ Exécuté":s==="PROCESSING"?"En cours":"En attente"):(s==="COMPLETED"?"✓ Executed":s==="PROCESSING"?"Processing":"Pending");
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-1 bg-white border border-gray-200 rounded-2xl p-1 mb-6 overflow-x-auto shadow-sm">
          {t.tabs.map(tab_=><button key={tab_.id} onClick={()=>setTab(tab_.id)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all whitespace-nowrap ${tab===tab_.id?"bg-emerald-600 text-white shadow-sm":"text-gray-600 hover:bg-gray-100"}`}>{tab_.icon} {tab_.label}{tab_.id==="fraud"&&unresolved>0&&<span className="bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{unresolved}</span>}</button>)}
        </div>

        {tab==="overview"&&<div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{[...t.stats,{label:lang==="fr"?"Alertes fraude":"Fraud alerts",v:unresolved,icon:"🔍"}].map(s=><div key={s.label} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm"><div className="text-3xl mb-2">{s.icon}</div><div className="text-2xl font-black text-gray-900">{s.v}</div><div className="text-xs text-gray-500 mt-0.5">{s.label}</div></div>)}</div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        {tab==="cases"&&<div className="space-y-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between"><h3 className="font-bold text-gray-900">{t.pendingTitle}</h3><Badge color="yellow">{pending.length} {t.pending}</Badge></div>
            {pending.length===0?<div className="p-14 text-center"><div className="text-5xl mb-3">✅</div><div className="font-bold text-gray-700">{t.empty}</div></div>:
            <div className="divide-y divide-gray-50">{pending.map(c=><div key={c.id} className="p-5 flex flex-col sm:flex-row sm:items-center gap-4"><div className="flex-1 min-w-0"><div className="font-semibold text-gray-900 mb-1">{c.title[lang]}</div><div className="flex flex-wrap gap-3 text-xs text-gray-500"><span>🏥 {c.hospital}</span><span>💰 {fmt(c.amount)}</span><span>📅 {c.submitted[lang]}</span></div></div><div className="flex items-center gap-3"><span className="text-xs text-gray-500">{t.risk}:</span><Badge color={c.risk<30?"green":c.risk<60?"yellow":"red"}>{c.risk}/100</Badge><button onClick={()=>setPending(p=>p.filter(x=>x.id!==c.id))} className="px-3 py-1.5 border border-red-200 text-red-600 rounded-xl text-xs font-bold hover:bg-red-50">{t.reject}</button><button onClick={()=>setPending(p=>p.filter(x=>x.id!==c.id))} className="px-3 py-1.5 bg-emerald-600 text-white rounded-xl text-xs font-bold hover:bg-emerald-700 shadow-sm">{t.approve}</button></div></div>)}</div>}
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div className="p-5 border-b border-gray-100"><h3 className="font-bold text-gray-900">{t.submittedCases} <Badge color="blue">{realCases.length}</Badge></h3></div>
            {loadingCases?<div className="p-10 text-center text-gray-400">{t.loadingCases}</div>:
             realCases.length===0?<div className="p-10 text-center text-gray-400">{t.noSubmissions}</div>:
             <div className="divide-y divide-gray-50">{realCases.map(c=><div key={c.id} className="p-5"><div className="flex items-start justify-between gap-3"><div><div className="font-semibold text-gray-900">{c.title}</div><div className="text-xs text-gray-500 mt-1">🏥 {c.hospital} · 📍 {c.city} · 💰 {fmt(c.amount||0)}</div><div className="text-xs text-gray-400 mt-0.5">{c.description?.slice(0,80)}...</div></div><Badge color={c.status==="PENDING"?"yellow":c.status==="APPROVED"?"green":"gray"}>{c.status||"PENDING"}</Badge></div></div>)}</div>}
          </div>
        </div>}

        {tab==="fraud"&&<div className="space-y-5">
          <div className="grid grid-cols-3 gap-4">{t.fraudLabels.map(fl=>{const count=fl.sev?alerts.filter(a=>!a.resolved&&a.sev===fl.sev).length:alerts.filter(a=>a.resolved).length;return<div key={fl.label} className={`bg-${fl.c}-50 border border-${fl.c}-200 rounded-2xl p-5 text-center`}><div className={`text-3xl font-black text-${fl.c}-700`}>{count}</div><div className={`text-xs text-${fl.c}-600 font-semibold mt-1`}>{fl.label}</div></div>;})}</div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm"><div className="p-5 border-b border-gray-100"><h3 className="font-bold text-gray-900">{t.fraudTitle}</h3></div><div className="divide-y divide-gray-50">{alerts.map(a=><div key={a.id} className={`p-5 flex items-center gap-4 ${a.resolved?"opacity-40":""}`}><div className={`w-1.5 h-10 rounded-full flex-shrink-0 ${a.sev==="critical"?"bg-red-500":a.sev==="high"?"bg-amber-500":"bg-yellow-400"}`}/><div className="flex-1 min-w-0"><div className="font-semibold text-sm text-gray-900">{a.type[lang]}</div><div className="text-xs text-gray-500 mt-0.5">{a.case[lang]} · {a.time}</div></div><Badge color={a.sev==="critical"?"red":a.sev==="high"?"yellow":"gray"}>{a.sev}</Badge>{!a.resolved?<button onClick={()=>setAlerts(al=>al.map(x=>x.id===a.id?{...x,resolved:true}:x))} className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl text-xs font-bold flex-shrink-0">{t.resolve}</button>:<Badge color="green">{t.resolved}</Badge>}</div>)}</div></div>
        </div>}

        {tab==="payouts"&&<div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="p-5 border-b border-gray-100 flex items-center justify-between"><h3 className="font-bold text-gray-900">{t.payoutsTitle}</h3><Badge color="yellow">{payouts.filter(p=>p.status==="PENDING").length} {t.payoutsPending}</Badge></div>
          <div className="divide-y divide-gray-50">{payouts.map(p=><div key={p.id} className="p-5 flex flex-col sm:flex-row sm:items-center gap-4"><div className="flex-1 min-w-0"><div className="font-semibold text-sm text-gray-900 truncate">{p.case[lang]}</div><div className="text-xs text-gray-500 mt-0.5">🏥 {p.hospital} · 📅 {p.date[lang]}</div></div><div className="flex items-center gap-3"><span className="font-bold text-gray-900">{fmt(p.amount)}</span><Badge color={p.status==="COMPLETED"?"green":p.status==="PROCESSING"?"blue":"yellow"}>{payoutLabel(p.status)}</Badge>{p.status==="PENDING"&&<button onClick={()=>setPayouts(pv=>pv.map(x=>x.id===p.id?{...x,status:"PROCESSING"}:x))} className="px-3 py-1.5 bg-emerald-600 text-white rounded-xl text-xs font-bold">{t.validate}</button>}</div></div>)}</div>
        </div>}
      </div>
    </div>
  );
...
export default function AyyadApp() {
  ...
  // Restaure la session Supabase au chargement
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <main>
        ...
        {page==="submit"&&<SubmitPage setPage={setPage} lang={lang} user={user} />}
